...
type PostfixExporter struct {
	...
	qmgrInsertsNrcptTypo prometheus.Histogram
...
func (e *PostfixExporter) collectQmgrLog(line, remainder, level string) {
	...
	case qmgrInsertMatches != nil:
		...
		addToHistogram(e.qmgrInsertsNrcptTypo, qmgrInsertMatches[2], "QMGR nrcpt")
...
func (e *PostfixExporter) init() {
	...
	e.once.Do(func() {
		...
		e.qmgrInsertsNrcptTypo = prometheus.NewHistogram(prometheus.HistogramOpts{
			Namespace:   "postfix",
...
func (e *PostfixExporter) Describe(ch chan<- *prometheus.Desc) {
	...
	ch <- e.qmgrInsertsNrcptTypo.Desc()
